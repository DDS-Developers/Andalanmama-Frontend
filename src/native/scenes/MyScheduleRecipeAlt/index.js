/**
 * scenes/MyScheduleRecipeAlt/index.js
 *
 */
...
export class MyScheduleRecipeAlt extends PureComponent {
  state = initialState;

  static navigationOptions = {
    title: 'Pilihan Buku Resep',
    header: null,
  };
  ...
  renderList = () => {
    const { items, loading, recipeOtherIds } = this.props;
    // const { items, loading, recipeOtherIds, recipeOthers } = this.props;
    // const count = recipeOthers.count();
    const count = items.count();
    const errorMax = recipeOtherIds.count() >= maxSelected;
          ...
          <ListRecipe
            ...
            onRenderItem={item => (
              <RecipeItem
                recipe={item}
                disable={errorMax}
                selected={recipeOtherIds.find(id => item.get('id') === id)}
                onAddRecipe={recipe => this.props.addRecipeOther(recipe)}
                onRemoveRecipe={recipeId => this.props.removeRecipeOther(recipeId)}
              />
            )}
            onLoadMore={paged => {
